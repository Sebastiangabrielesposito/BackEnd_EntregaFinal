...
import {createCar} from '../controllers/carts.controller.js';
import {createCarInUserRegister} from '../controllers/carts.controller.js';
  ...
  new LocalStrategy(
    ...
    async (req, email, password, done) => {
      ...
      try {
        ...
        if (email === "adminCoder@hotmail.com"  && password === config.ADMIN_CODER_KEY) {
          
          newUserBd.role = 'admin'
        } else if (email.includes("@premium.com")) {
          newUserBd.role = 'premium';
        } else {
          newUserBd.role = 'user';
        }
        await newUserBd.save()
        const usersRespDTOs = new UsersRespDTO(newUserBd)
        // logger.info(usersRespDTOs);
        logger.info(JSON.stringify(usersRespDTOs));
        userId = newUserBd._id;
        await createCarInUserRegister(userId, req);
        done(null, newUserBd);
      } catch (error) {
        done(error)
      }
  ...
  new LocalStrategy(
    ...
    async (req, email, password, done) => {
      try {
        ...
        if (email ===  config.ADMIN_CODER_USER && password ===  config.ADMIN_CODER_KEY) {
